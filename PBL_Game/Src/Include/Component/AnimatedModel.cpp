...
ModelMesh::AnimatedMesh AnimatedModel::processMesh(aiMesh *mesh, const aiScene *scene)
{
    // data to fill
    std::vector<ModelMesh::Vertex> vertices;
    std::vector<unsigned > indices;
    std::vector<ModelMesh::Texture> textures;
    std::vector<ModelMesh::VertexBoneData> bones_id_weights_for_each_vertex;
    ...
    return ModelMesh::AnimatedMesh(vertices, indices, textures,bones_id_weights_for_each_vertex, ShaderProgram);
}

std::vector<ModelMesh::Texture> AnimatedModel::loadMaterialTextures(const aiScene *scene,aiMaterial *mat, aiTextureType type, std::string typeName)
{
    std::vector<ModelMesh::Texture> textures;
