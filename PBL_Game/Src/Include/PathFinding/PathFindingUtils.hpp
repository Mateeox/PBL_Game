...
static bool operator==(GridLocation a, GridLocation b)
{
...
static std::basic_iostream<char>::basic_ostream &operator<<(std::basic_iostream<char>::basic_ostream &out, const GridLocation &loc)
{
  out << '(' << loc.x << ',' << loc.y << ')';
  return out;
}
...
static void add_rect(SquareGrid &grid, int x1, int y1, int x2, int y2)
{
  for (int x = x1; x < x2; ++x)
  {
    for (int y = y1; y < y2; ++y)
...
static void add_Wall(SquareGrid &grid, int x, int y)
{
   grid.walls.insert(GridLocation{x, y});  
}
...
static GridWithWeights make_diagramFromGeneratedMap(int size_x,int size_y)
{
  GridWithWeights grid(size_x, size_y);
  add_rect(grid, 1, 7, 4, 9);
  typedef GridLocation L;
